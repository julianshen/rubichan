...
**The portable core now honors all four seams.** An out-of-module program can attach dynamic prompt content, concurrent work, and tool-execution wrappers to the real core with no `internal/` dependency. The remaining step for the end-to-end proof is rebuilding `examples/embed` on `agentsdk.NewAgent` — it currently composes `internal/agent.New`, so it still demonstrates only in-module embedding.
