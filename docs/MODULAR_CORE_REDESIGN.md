...
>
> **Option 2 deserves weight it has not been given.** The adapters are 991 lines that no production path exercises, whose tests never send them a request, and three quarters of whose operations would fail if they did. That is not scaffolding awaiting a capability; it is code that has never worked against the thing it targets. Deleting it is a legitimate outcome of this audit, and much cheaper than the migration the doc has been assuming.
>
