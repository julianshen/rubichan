...
>
> **Measured: 3,383 lines before slice 1 → 2,621 now.** That is 762 lines across all five slices (slice 5 alone accounts for 2,795 → 2,621), against a target of "a few hundred" — i.e. roughly 19% of the distance, and the remaining ~2,500 lines are the harder part (the inline interactive/headless/wiki flows identified in Problem C, which need the dormant `internal/modes/*` adapters to become the real path). Slice 4 also dropped **nine imports** from main.go — the four `builtin/` skill packages, all four skill backends, and `pkg/skillsdk` — which is the better measure of what moved: main.go no longer knows those subsystems exist. The line count moves independently of this work too: #329's Ollama fix added 17 lines between slices 1 and 2. Treat the number as a direction of travel, not a burn-down.
>
...
>
> **Slice 5 is the first where duplication had already become a defect, and that is the fourth pattern.** The subagent block existed in both long mode functions and had diverged three ways. One divergence was live: `runHeadless` discarded the worktree manager `setupWorkingDir` returned and built a second one over the same repository, so `--headless --worktree` ran two managers, each enforcing `MaxWorktrees` against its own count, neither aware of the other's worktrees. A second was quieter — interactive discarded agent-definition registration errors that headless logged, so a duplicate name produced a subagent the user could not spawn and no explanation.
>
> The sequencing matters and is worth repeating on later slices: **the extraction parameterised every divergence rather than picking a winner**, so the move stayed behaviour-preserving and each fix landed afterwards as a reviewable `[BEHAVIORAL]` commit with a test that fails without it. Extracting and fixing in one step would have hidden two behaviour changes inside a refactor.
>
> The general point: **two copies of a block in two long functions will drift, and nothing surfaces it until they are forced into one place.** Slice 4's duplication was still benign; this one had already cost something. The remaining mode-flow work should assume more of it.
