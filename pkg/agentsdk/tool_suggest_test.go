...
func TestSuggestToolName(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.unknown, func(t *testing.T) {
			result := agentsdk.SuggestToolName(tt.unknown, available)
...
func TestSuggestToolNameEmptyAvailable(t *testing.T) {
	assert.Equal(t, "", agentsdk.SuggestToolName("shell_exec", nil))
	assert.Equal(t, "", agentsdk.SuggestToolName("shell_exec", []string{}))
}

func TestSuggestToolNameCaseInsensitive(t *testing.T) {
	available := []string{"shell", "file"}
	assert.Equal(t, "shell", agentsdk.SuggestToolName("SHELL_EXEC", available))
	assert.Equal(t, "file", agentsdk.SuggestToolName("FILE_WRITE", available))
}
