...
func TestApprovalPromptView(t *testing.T) {
	ap := NewApprovalPrompt("shell", `{"command":"rm -rf /tmp"}`, "", 60, nil, false)
	view := ap.View()
...
func TestApprovalPromptViewContainsArgs(t *testing.T) {
	ap := NewApprovalPrompt("file", `{"operation":"read","path":"/etc/passwd"}`, "", 80, nil, false)
	view := ap.View()
...
func TestApprovalPromptMinWidth(t *testing.T) {
	...
	ap := NewApprovalPrompt("shell", `{"command":"ls"}`, "", 10, nil, false)
	view := ap.View()
	assert.Contains(t, view, "Bash")
}
...
func TestApprovalPromptMediumRiskLabel(t *testing.T) {
	ap := NewApprovalPrompt("file_write", `{"operation":"write","path":"/tmp/foo.txt"}`, "", 60, nil, false)
	view := ap.View()
	assert.Contains(t, view, "●")
...
func TestApprovalPromptLowRiskLabel(t *testing.T) {
	ap := NewApprovalPrompt("file_read", `{"operation":"read","path":"/tmp/foo.txt"}`, "", 60, nil, false)
	view := ap.View()
	assert.Contains(t, view, "●")
...
func TestApprovalPromptSanitizesANSI(t *testing.T) {
	...
	ap := NewApprovalPrompt("\x1b[31mfake_safe\x1b[0m", `"\x1b[32msafe_args\x1b[0m"`, "", 80, nil, false)
	view := ap.View()
...
func TestApprovalPromptCustomOptions(t *testing.T) {
	...
	ap := NewApprovalPrompt("file_read", `{"path":"/etc/passwd"}`, "", 60, opts, false)
	view := ap.View()
	assert.Contains(t, view, "[Y]")
	assert.Contains(t, view, "[N]")
	assert.NotContains(t, view, "[A]")
...
func TestApprovalPromptShowsDenyOption(t *testing.T) {
	...
	ap := NewApprovalPrompt("file_read", `{"path":"foo.go"}`, "", 60, nil, false)
	view := ap.View()
...
func TestApprovalPromptView_NoRawJSON(t *testing.T) {
	ap := NewApprovalPrompt("shell", `{"command":"git status","description":"Check working tree"}`, "", 80, nil, false)
	view := ap.View()
...
func TestApprovalPromptView_FileShowsPath(t *testing.T) {
	ap := NewApprovalPrompt("file", `{"operation":"read","path":"src/main.go"}`, "", 80, nil, false)
	view := ap.View()
...
func TestApprovalPromptView_SearchShowsPattern(t *testing.T) {
	ap := NewApprovalPrompt("search", `{"pattern":"func Test","path":"internal/"}`, "", 80, nil, false)
	view := ap.View()
...
func TestApprovalPromptView_WorkDirShownForHighRisk(t *testing.T) {
	t.Parallel()
	ap := NewApprovalPrompt("shell", `{"command":"ls"}`, "/home/user/project", 80, nil, false)
	view := ap.View()
...
func TestApprovalPromptView_WorkDirHiddenForLowRisk(t *testing.T) {
	t.Parallel()
	ap := NewApprovalPrompt("file_read", `{"path":"foo.go"}`, "/home/user/project", 80, nil, false)
	view := ap.View()
...
func TestApprovalPromptView_EmptyWorkDirNotShown(t *testing.T) {
	t.Parallel()
	ap := NewApprovalPrompt("shell", `{"command":"ls"}`, "", 80, nil, false)
	view := ap.View()
...
func TestApprovalPrompt_BatchHintNotShownWhenFalse(t *testing.T) {
	ap := NewApprovalPrompt("shell", `{"command":"ls"}`, "", 80, nil, false)
	view := ap.View()
	assert.NotContains(t, view, "batch")
}

func TestApprovalPrompt_BatchHintShownWhenTrue(t *testing.T) {
	ap := NewApprovalPrompt("shell", `{"command":"ls"}`, "", 80, nil, true)
	view := ap.View()
	assert.Contains(t, view, "batch")
}

func TestApprovalPrompt_BatchKeyHandling(t *testing.T) {
	ap := NewApprovalPrompt("shell", `{"command":"ls"}`, "", 80, nil, true)
...
func TestApprovalPrompt_BatchKeyIgnoredWhenHintDisabled(t *testing.T) {
	ap := NewApprovalPrompt("shell", `{"command":"ls"}`, "", 80, nil, false)
	handled := ap.HandleKey(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune("b")})
	assert.False(t, handled)
	assert.False(t, ap.Done())
...
func TestApprovalPrompt_ViewportForLongArgs(t *testing.T) {
	// Create args with > 5 lines when formatted.
	// formatToolArgs will extract command field and potentially add description.
	longArgs := `{
	"command": "line1\nline2\nline3\nline4\nline5\nline6\nline7",
	"description": "This is a description"
}`
	ap := NewApprovalPrompt("shell", longArgs, "", 80, nil, false)
	assert.True(t, ap.useViewport, "long args should enable viewport")
	assert.NotNil(t, ap.argsViewport)
}

func TestApprovalPrompt_NoViewportForShortArgs(t *testing.T) {
	shortArgs := `{"command":"ls"}`
	ap := NewApprovalPrompt("shell", shortArgs, "", 80, nil, false)
	assert.False(t, ap.useViewport, "short args should not enable viewport")
}
